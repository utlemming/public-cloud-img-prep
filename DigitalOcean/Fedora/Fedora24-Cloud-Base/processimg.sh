...
docker run -i --rm --privileged -v /tmp/doimg:/tmp/doimg fedora:23 bash << 'EOF'
set -eux
WORKDIR=/workdir
TMPMNT=/workdir/tmp/mnt

# Vars for the image
XZIMGURL='https://kojipkgs.fedoraproject.org//work/tasks/4918/14404918/Fedora-Cloud-Base-24-20160607.n.0.x86_64.raw.xz'
XZIMG=$(basename $XZIMGURL) # Just the file name
XZIMGSUM='9b419360991c4044127a1c4d3d938b43d9ae247c5b5a7490fadfc295cb89347a'
...
vendor_data:
   enabled: True
END
# Don't know why this one is failing with "No such file or directory"
#chcon system_u:object_r:etc_t:s0 ${TMPMNT}/${DOCLOUDCFGFILE}
# Try doing it this way instead
chcon -v --reference=${TMPMNT}/etc/fstab ${TMPMNT}/${DOCLOUDCFGFILE}
